...
export function getPosFromCountry(countryName) {
  for (const record of countryPos) {
    const [country, lon, lat] = record;
    if (
      country.toString().toLowerCase() === countryName.toString().toLowerCase()
    ) {
      return [lat, lon];
    }
  }
